...
def test_analysis_golden(case):
    metrics = analysis.analyze_series(case["data"], granularity="monthly")
...
def test_empty_series_reports_unavailable():
    ...
    assert analysis.analyze_series(Series()) == {"available": False}


def test_share_per_million_aligns_on_dates():
    df = _monthly([100, 200, 300])
    agg = _monthly([1_000_000, 1_000_000, 1_000_000])
...
def test_share_per_million_none_on_empty():
    ...
    assert analysis.share_per_million(_monthly([100]), Series()) is None
